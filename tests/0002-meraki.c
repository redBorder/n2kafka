...
static void MerakiDecoder_default_secret_hit() {
	...
	json_t *meraki_secrets_array = json_loadb(MERAKI_SECRETS_DEFAULT_IN,strlen(MERAKI_SECRETS_DEFAULT_IN),0,&jerr);
	assert(meraki_secrets_array);
	const int parse_rc = parse_meraki_secrets(&meraki_config.database, meraki_secrets_array);
	assert(parse_rc == 0);
	json_decref(meraki_secrets_array);
...
static void MerakiDecoder_default_secret_miss() {
	...
	json_t *meraki_secrets_array = json_loadb(MERAKI_SECRETS_DEFAULT_OUT,strlen(MERAKI_SECRETS_DEFAULT_OUT),0,&jerr);
	assert(meraki_secrets_array);
	const int parse_rc = parse_meraki_secrets(&meraki_config.database, meraki_secrets_array);
	assert(parse_rc == 0);
	json_decref(meraki_secrets_array);
